...
def normalbetrieb(zeile, menge, std_dz, std_la, std_vb, std_ls, data_id_nr, std_te, starttime):
    ...
    while x < y:
        ...
        d.write("ABC" + str(data_id_nr) + ";" + str(time) + ";" + str(drehzahl) + ";" + str(leistungsaufnahme)  + ";" + str(vibration)  + ";" + str(lautstaerke) + ";" + str(std_te) + ";" + "leer\n")
        zeile = zeile + 1
        x = x + 1
    # print("Normalbetrieb")
    return zeile
...
def wartung(zeile, menge, std_dz, std_la, std_vb, std_ls, data_id_nr, std_te, starttime):
    ...
    while x < y:
        ...
        time = starttime + timedelta(seconds = 30 * zeile)
        d.write("ABC" + str(data_id_nr) + ";" + str(time) + ";" + str(drehzahl) + ";" + str(leistungsaufnahme)  + ";" + str(vibration)  + ";" + str(lautstaerke) + ";" +  str(std_te) + ";" + str(fehler_id) +  "\n")
        zeile = zeile + 1
        x = x + 1
    # print("Wartung")
    return zeile
...
def ausfall_2(zeile, menge, std_dz, std_la, std_vb, std_ls, data_id_nr, std_te, starttime):
    ...
    while temperatur < 200:
        ...
        d.write("ABC" + str(data_id_nr) + ";" + str(time) + ";" + str(drehzahl) + ";" + str(leistungsaufnahme)  + ";" + str(vibration)  + ";" + str(lautstaerke) + ";" + str(temperatur) + ";" + str(fehler_id) + "\n")
        zeile = zeile + 1
...
def choose(zeile, menge, std_dz, std_la, std_vb, std_ls, data_id_nr, std_te, starttime):
    random_choice = random.randrange(1, 100)
    if random_choice < 60:
        zeile = str(normalbetrieb(zeile, menge, std_dz, std_la, std_vb, std_ls, data_id_nr, std_te, starttime))   
    elif random_choice < 80:
        zeile = str(wartung(zeile, menge, std_dz, std_la, std_vb, std_ls, data_id_nr, std_te, starttime))
    elif random_choice < 90:
        zeile = str(ausfall_1(zeile, menge, std_dz, std_la, std_vb, std_ls, data_id_nr, std_te, starttime))
    else:
        zeile = str(ausfall_2(zeile, menge, std_dz, std_la, std_vb, std_ls, data_id_nr, std_te, starttime))
    #print(zeile)
    return zeile

# Aufruf Auswahl
while zeile < menge:
    zeile = int(choose(zeile, menge, std_dz, std_la, std_vb, std_ls, data_id_nr, std_te, starttime))
